// Autonomous studio smoke: login â†’ navigate â†’ submit "simple counter" â†’ watch for ready_set.
const { chromium } = require('playwright');

const STUDIO_URL = 'http://localhost:5183';
const PROMPT = 'counter';
const TOTAL_TIMEOUT_MS = 240_000; // 3 minutes for ready_set
...
(async () => {
  ...
  try {
    ...
    if (await loginBtn.count().catch(() => 0)) {
      console.log('Clicking Test Loginâ€¦');
    ...
    console.log('Textarea found. Filling promptâ€¦');
    textarea = page.locator('textarea').first();
    await textarea.click();
    await textarea.fill(PROMPT);

    // Submit â€” prefer Enter (onKeyDown handler fires onSend)
    console.log('Submitting prompt via Enterâ€¦');
    await textarea.press('Enter');

    console.log('Watching for ready_set up to 3 minutesâ€¦');
