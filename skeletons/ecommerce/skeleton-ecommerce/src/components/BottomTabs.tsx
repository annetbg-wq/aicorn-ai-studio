  ...
  return (
    ...
    >
      <ul className="grid grid-cols-5 px-2 pt-1.5">
        {BOTTOM_TABS.map((tab) => {
          const count =
            tab.badge === 'cart' ? cart.itemCount :
            tab.badge === 'wishlist' ? wishlist.count : 0;
                    ...
                    <div className="relative">
                      ...
                      {count > 0 && (
                        <span
                          ...
                          className="absolute -right-2 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-primary px-1 text-[10px] font-semibold text-primary-foreground"
                        >
                          {count > 9 ? '9+' : count}
                        </span>
                      )}
